...
Non-trivial problems solved in past sessions are written up in `docs/solutions/` (categorized, with YAML frontmatter). Search there before re-deriving a fix or pattern: `grep -ri <keyword> docs/solutions/`. When you solve one, write it up there directly — that is the one knowledge store besides this file.

## Worktrees

Worktrees live in `.claude/worktrees/` within the repo. Claude Code: `/wt <feature name>` creates one, `/dwt` cleans up; any agent: `git worktree add .claude/worktrees/<name> -b <branch> origin/master`. Branch from `master`, PR back to `master`. Run the backend with the main checkout's absolute venv path (`<main clone>/.venv/bin/python`), never a relative `../../.venv` — worktrees sit two levels deeper, so the relative path resolves to nothing.
